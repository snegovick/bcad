...
import math
from bsuite.bsk.calc_utils import AABB, CircleUtils, LineUtils, ArcUtils, PointUtils, vect_len, mk_vect
...
class Element(object):
    ...
    def get_snap_points(self):
        ...
        if Singleton.state.settings.is_centerpoint_snap_enabled():
            points = self.append_centerpoint(points)

        points = self.append_start(points)
        points = self.append_end(points)
        return points

    def append_centerpoint(self, points):
        return points

    def append_start(self, points):
        return points

    def append_end(self, points):
        return points
...
class ELine(Element):
    ...
    def to_scl(self):
        return "line(start=%f, end=%f, color=%s);"%(self.start, self.end, self.color)

    def draw_first(self, ctx):
        offset = Singleton.state.get_offset()
        scale = Singleton.state.get_scale()

        sx = self.start[0]*scale[0]+offset[0]
        sy = offset[1]-self.start[1]*scale[1]
        ex = self.end[0]*scale[0]+offset[0]
        ey = offset[1]-self.end[1]*scale[1]
        debug("oy: %f, sy: %f"%(self.start[1], sy))
